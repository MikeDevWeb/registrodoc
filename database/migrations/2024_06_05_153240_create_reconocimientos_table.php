...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('reconocimientos', function (Blueprint $table) {
            ...
            $table->string('reconocimiento',120) ->pattern('[a-zA-Z]+') -> required;
            $table->string('institucion',50) ->pattern('[a-zA-Z]+') -> nullable;
            $table->string('anio') ->pattern('/^[0-9]+/') ->min(1970) ->max(date('Y')) -> required;
            $table->string('actividad', 50) ->pattern('[a-zA-Z]+') -> required;
