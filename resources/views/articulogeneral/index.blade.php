                                ...
                                <table id="tablita" class="table table-striped table-hover text-xs w-100">
                                    ...
                                    <tbody>
                                        @foreach ($articulogenerals as $articulogeneral)
                                            <tr>
                                                <td>{{ ++$i }}</td>
                                                <td >{{ $articulogeneral->datospersona->nombre }} {{ $articulogeneral->datospersona->apellidoPaterno }} {{ $articulogeneral->datospersona->apellidoMaterno }}</td>
                                                <td >{{ Str::limit($articulogeneral->nombrearticulo, 35)}}</td>
                                                ...
                                                <td >{{ $articulogeneral->user->name }}</td>
                                                <td >{{ $articulogeneral->fecharegistro }}</td>
                                                <td>
                                                    <form action="{{ route('articulogenerals.destroy', $articulogeneral->id) }}" method="POST">
                                                        <div class="btn-group">
                                                            <a class="btn-xs btn-primary text-center" href="{{ route('articulogenerals.show', $articulogeneral->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Revisar') }}</a>
                                                            <a class="btn-xs btn-success text-center" href="{{ route('articulogenerals.edit', $articulogeneral->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Modificar') }}</a>
                                                            @csrf
                                                            @method('DELETE')
