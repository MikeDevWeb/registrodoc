    ...
    <div class="col-md-12 row">
        <div class="col-md-6">
            <div class="form-group mb-2 mb20 text-sm">
                ...
                <select class="text-sm form-control text-sm text-sm text-sm @error('datospersona_id') is-invalid @enderror" name="datospersona_id" id="datospersona_id">
                    ...
                    @foreach ($datospersonas as $datospersona)
                        @php
                            $isRegistered = \App\Models\Expprograrel::where('datospersona_id', $datospersona->id)
                                ->where('id', '<>', optional($expprograrel)->id)
                                ->exists();
                        @endphp
                        <option value="{{ $datospersona->id }}" {{ $datospersona->id == (old('datospersona_id', $expprograrel?->datospersona_id) ?: '') ? 'selected' : '' }} {{ $isRegistered ? 'disabled' : '' }}>
            ...
            <div class="form-group mb-2 mb20 text-sm">
                ...
                <input type="text" name="institucion" class="form-control text-sm @error('institucion') is-invalid @enderror" value="{{ old('institucion', $expprograrel?->institucion) }}" id="institucion" placeholder="Institucion">
                {!! $errors->first('institucion', '<div class="invalid-feedback" role="alert"><strong>:message</strong></div>') !!}
            </div>
            <div class="form-group mb-2 mb20 text-sm">
                <label for="cargoactividad" class="form-label text-sm">{{ __('Cargo o actividad') }}</label>
                <input type="text" name="cargoactividad" class="form-control text-sm @error('cargoactividad') is-invalid @enderror" value="{{ old('cargoactividad', $expprograrel?->cargoactividad) }}" id="cargoactividad" placeholder="Cargoactividad">
            ...
            <div class="form-group mb-2 mb20 text-sm">
                ...
                <input type="text" name="duracion" class="form-control text-sm @error('duracion') is-invalid @enderror" value="{{ old('duracion', $expprograrel?->duracion) }}" id="duracion" placeholder="Duracion">
        ...
        <div class="col-md-6">
            <div class="form-group mb-2 mb20 text-sm">
                ...
                <input type="date" name="fechainicio" class="form-control text-sm @error('fechainicio') is-invalid @enderror" value="{{ old('fechainicio', $expprograrel?->fechainicio) }}" id="fechainicio" placeholder="Fechainicio">
            ...
            <div class="form-group mb-2 mb20 text-sm">
                ...
                <input type="date" name="fechafin" class="form-control text-sm text-sm @error('fechafin') is-invalid @enderror" value="{{ old('fechafin', $expprograrel?->fechafin) }}" id="fechafin" min="{{ $formcurso?->fechainicio ?? '' }}">
                @error('fechafin')
                    <div class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></div>
                @enderror
                @if($expprograrel?->fechainicio && $expprograrel?->fechainicio > $expprograrel?->fechafin)
                    <span class="text-danger">La fecha final debe ser mayor que la fecha inicial.</span>
                @elseif($expprograrel?->fechainicio && $expprograrel?->fechainicio < $expprograrel?->fechafin)
            ...
            <div class="form-group mb-2 mb20 text-sm">
                ...
                <input title="Registrado por" readonly hidden required type="text" name="user_id" class="form-control text-sm text-sm text-sm @error('user_id') is-invalid @enderror" value="{{ auth()->user()->id }}" id="user_id">
            ...
            <div class="form-group mb-2 mb20 text-sm text-sm">
                ...
                <input readonly required type="text" name="fecharegistro" class="form-control text-sm @error('fecharegistro') is-invalid @enderror" value="{{ old('fecharegistro', $expprograrel?->fecharegistro ?? \Carbon\Carbon::now()->format('Y-m-d')) }}" id="fecharegistro">
