...
@section('template_title')
    Experiencia en programas relacionados
@endsection
                    ...
                    <div class="card-header text-sm">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <span id="card_title  text-sm">
                                {{ __('Experiencia en programas relacionados') }}
                            </span>
                                ...
                                <table id="tablita" class="table table-striped table-hover text-xs w-100">
                                    ...
                                    <tbody>
                                        @foreach ($expprograrels as $expprograrel)
                                            <tr>
                                                <td>{{ ++$i }}</td>
                                                <td >{{ $expprograrel->datospersona->nombre }} {{ $expprograrel->datospersona->apellidoPaterno }} {{ $expprograrel->datospersona->apellidoMaterno }}</td>
                                                ...
                                                <td >{{ $expprograrel->user->name }}</td>
                                                <td >{{ $expprograrel->fecharegistro }}</td>
                                                <td>
                                                    <form action="{{ route('expprograrels.destroy', $expprograrel->id) }}" method="POST">
                                                        <div class="btn-group">
                                                            <a class="btn-xs btn-primary text-center" href="{{ route('expprograrels.show', $expprograrel->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Revisar') }}</a>
                                                            <a class="btn-xs btn-success text-center" href="{{ route('expprograrels.edit', $expprograrel->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Modificar') }}</a>
                                                            @csrf
                                                            @method('DELETE')
