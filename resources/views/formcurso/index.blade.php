                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach ($formcursos as $formcurso)
										...
										<td >{{ $formcurso->datospersona_id }}</td>
										<td >{{ $formcurso->user_id }}</td>

                                            <td>
                                                <form action="{{ route('formcursos.destroy', $formcurso->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('formcursos.show', $formcurso->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Show') }}</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('formcursos.edit', $formcurso->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Edit') }}</a>
                                                    @csrf
                                                    @method('DELETE')
