                                ...
                                <table id="tablita" class="table table-striped table-hover text-xs w-100">
                                    ...
                                    <tbody>
                                        @foreach ($libropublicados as $libropublicado)
                                            <tr>
                                                <td>{{ ++$i }}</td>
                                                <td >{{ $libropublicado->datospersona->nombre }} {{ $libropublicado->datospersona->apellidoPaterno }} {{ $libropublicado->datospersona->apellidoMaterno }}</td>
                                                <td >{{ $libropublicado->titulo }}</td>
                                                <td >{{ $libropublicado->anio }}</td>
                                                <td >{{ $libropublicado->autor }}</td>
                                                <td >{{ $libropublicado->coautor }}</td>
                                                <td >{{ $libropublicado->fecharegistro }}</td>
                                                <td >{{ $libropublicado->user->name }}</td>
                                                <td>
                                                    <form action="{{ route('libropublicados.destroy', $libropublicado->id) }}" method="POST">
                                                        <div class="btn-group">
                                                            <a class="btn-xs btn-primary text-center" href="{{ route('libropublicados.show', $libropublicado->id) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Revisar') }}</a>
                                                            <a class="btn-xs btn-success text-center" href="{{ route('libropublicados.edit', $libropublicado->id) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Modificar') }}</a>
                                                            @csrf
                                                            @method('DELETE')
